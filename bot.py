...
QTY = 0.04
LEVERAGE = 45
CHECK_INTERVAL = int(os.environ.get("CHECK_INTERVAL", "3600"))  # seconds (default: 1 hour)
...
# Pause after volatility spikes (ATR as % of price)
...
def run_cycle():
    ...
    if atr_pct >= VOL_SPIKE_ATR_PCT:
        ...
        if pause_until > int(state.get("paused_until") or 0):
            ...
            state["pause_reason"] = f"VOL_SPIKE ({DECISION_INTERVAL}m ATR%={atr_pct*100:.2f}%)"
            save_state(state)
            send_telegram(
        ...
        # Return early — don't trade during a volatility spike
        return
    ...
    if state.get("had_position") and not position:
        if equity is not None and state.get("entry_equity") is not None:
            ...
            trade_hist = state.get("trade_history") or []
            trade_hist.append({
            ...
            state["trade_history"] = trade_hist[-50:]
            append_log("CLOSE", {
                "side": state.get("entry_side"),
                "entry_time": state.get("entry_time"),
                "exit_time": now,
    ...
    if position:
        ...
        # Store live position info in state so dashboard can show it
        state["live_pnl"] = round(pnl, 4)
        state["position_side"] = side
        state["entry_price_live"] = entry
        ...
        # No position — clear live position fields
        state["live_pnl"] = None
        state["position_side"] = None
        state["entry_price_live"] = None
        state["mark_price"] = None
        save_state(state)

    # --- Build indicators for Claude ---
    ...
    macd_line, sig_line, macd_hist = calculate_macd(closes_tf)
